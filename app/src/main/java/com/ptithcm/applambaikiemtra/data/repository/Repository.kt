...
class Repository(val data: firestore, val dao: AppDao) {
    ...
    fun getDataCauHoiFromApiToSQL(boMon: String, deThi: String, x: (MutableList<BaiThi>) -> Unit) {
        data.getCauHoi(
            boMon,
            deThi
        ) { data ->  // dùng con trỏ hàm mà ở tầng firestore đã dùng để nhận dx dữ liệu từ firebase
    ...
    fun updateDeThi(deThi: DeThi) {
     ...
     fun getDataFromFireStore(boMon: String, xx: (MutableList<DeThi>) -> Unit) {
        data.getDeBai(boMon) {
            val temp = MutableLiveData<MutableList<DeThi>>()
            for (x in it) {
